# <center>DEVELOP A STREAM HANDLER FOR CISCO CSA(CONNECTED STREAMING ANALYTICS)</center>
...
This learning lab can be used as a guide to get a high level understanding to develop CSA custom handlers for using CSA in  DLP(DevNet Data Learning Platform) platform. DLP platform has Kafka messaging framework that works from the backend. This utility takes the IP, Port and the topic details of Kafka messaging system. It listens to a local socket continuously and pushes the stream from the local socket to the Kafka. We will be using DevNet Data Learning Platform referred as "DLP" during the course. 
