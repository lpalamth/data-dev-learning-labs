...
We need to stop the handler now. Please refer the screenshot below:
...
Select "Build" from the IDE combo box to start the handler. Please refer the screenshot below:
...
Now we have completed all the configurations required as well as the coding. We need to visualize the same using Zeppelin
</br></br>
