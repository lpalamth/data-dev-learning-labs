...
**Preparing Python code for data generation**
Input the code as given below. After inputting code in Zeppelin UI, click on the <b>Run</b> button to execute the code.
```json
%python
...
iii) iii) Define an array y=cos(x). </br>

</br>
After executing the above code, user can view the cosine signal data in Zeppeling.
