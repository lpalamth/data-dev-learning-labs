...
From Learning Labs pane, select the learning lab "" and click on "Start" button as shown in screenshot below:
...
* The eclipse task(in cloudIDE pane) should be in started status and the colour of the icon should be Green. Please refer screenshot below:
...
Select the file “TransformData.java” from left pane on IDE work space. Double click on the file and it will open up in the right panel. Please refer the screenshot below:
...
After executing the above program, DLP's Cloud IDE will generate the output file and save directly in the Hadoop platform. We can preview the output file and also view using visualisation tool - Tableau. The preview option is shown below:
...
PS: The pop-up may be blocked in browser configuration. Please refer the screenshot below:
