...
If Java is not installed, then from the below-mentioned URL, download Java and installed it first. </br>
...
**Job Designe::** Job Creation Panel for creating a new transformation job. Details of creating new Talend job will be described in next steps.</br>
...
**Talend Components Palette::** Talend Open source Big data tool provides 900+ components for different transformation operation. User can select component(s) from this palette and place those on the canvas area and connect them. Execution flow of job should be done based on the connection of each component which describe the data flow also.
</br>
