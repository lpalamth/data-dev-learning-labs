...
**Section C: Build Transformation Job using Talend Big Data Tool in user's local computer.** </br>
In this section, the user will learn the process of building and exporting job which user has created already in previous section from Talend tool. 
...
5) Above step will upload the job in hadoop environment from your local machine and extract it for execution. This process will take 3/4 minutes to finish. A successfull promot message will apear to notify that job upload process is done successful. Sit back until you received the successful message.
