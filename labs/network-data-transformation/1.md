...
<b>N.B.</b> DLP platform provide a default Kafka's Producer, Consumer and Network traffic generator. User can select their own network data and use DLP provided Kafka's component to process the network data. User can define and deploy their own service also. 

# Step 1: Explore Data Learning Platform (DLP)

Workspace is a working zone for developer. For each programming practice, developer have to create different workspace from where user can start practicing the programming languages which can work on top of Hadoop platform using Cloud based IDE. 
